...
TEST_CASE(VectorMagnitudeNormalize) {
	...
	ASSERT_CONDITION(equal(normalize(vec2{ 5, 5 }), vec2{0.707106, 0.707106}));

	ASSERT_CONDITION(equal(normalize(vec3{ 5, 0, 0 }), vec3{1, 0, 0}));
	ASSERT_CONDITION(equal(normalize(vec3{ 5, 5, 0 }), vec3{0.707106, 0.707106, 0}));
	ASSERT_CONDITION(equal(normalize(vec3{ 5, 0, 5 }), vec3{0.707106, 0, 0.707106}));
}
